   ...
   """
    ...
    while  not (i == 1 and j ==1) :#It stops only when it reaches exactly index i = 1, j = 1
        ...
        if(i == 1 and j != 1):#base case for the first row
            ...
            matrix[0][j] = str(matrix[0][j]) + "*" #To mark the letters that have been labeled
            j = j-1
            continue
        if(i != 1 and j == 1):#base case for the first column
            lstr2.insert(0,"-")
            lstr1.insert(0,matrix[i][0])
            matrix[i][0] = str(matrix[i][0]) + "*" #To mark the letters that have been labeled
            i = i-1
            continue
        ...
        matrix[i][j] = str(matrix[i][j]) + "*" #To mark the backtracking trajectory
